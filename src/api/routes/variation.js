...
export async function variationRoutes(app, { queue }) {
  app.post('/variation', async (request, reply) => {
    ...
    if (parentJob.type !== 'imagine') {
      return reply.code(400).send({ error: 'Can only create variations from imagine jobs' });
    }
    ...
    return reply.code(202).send({
      job_id: id,
      status: 'pending',
      message: 'Variation job submitted',
    });
  });
}
